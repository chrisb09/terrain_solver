...
# they are present even if sbatch environment export is restricted.
{
  ...
  echo "export CHUNK_SIZE_ENV=${CHUNK_SIZE_ENV}"
  ...
  if [[ -n "${CPP_ML_CONFIG_ENV:-}" ]]; then
    echo "export CPP_ML_CONFIG_ENV=\"${CPP_ML_CONFIG_ENV}\""
  fi
