...
############################
# Single Node: GPU (c23g) 96 CPU cores, using 1 GPU
############################
...
if [[ "${SKIP_COMPILE}" -eq 1 ]]; then
  ...
  if [[ "${USE_CPP_ML_INTERFACE}" -eq 1 && "${CPP_ML_INTERFACE_PROVIDER:u}" == "PHYDLL" ]]; then
    ...
    NP_PHY="${MPI_RANKS}"
    PHYDLL_NP_DL="${PHYDLL_NP_DL_ENV:-${PHYDLL_NP_DL:-1}}"
    PHYDLL_DL_NODES="${PHYDLL_DL_NODES_ENV:-${PHYDLL_DL_NODES:-${PHYDLL_NP_DL}}}"
    ...
    if [[ "${DB_HET_GROUP}" -eq "${SOLVER_HET_GROUP}" ]]; then
      ...
      launch_cmd="srun --wait=120 ${srun_solver_args} \
        --cpus-per-task 1 \
        ${SRUN_DIST} \
        ${SOLVER_EXE} \
        --device \"${device}\" \
    ...
    srun_cmd="srun --wait=120 --export=ALL $(get_srun_het_flag "${SOLVER_HET_GROUP}") --nodes \"${_nodes:-1}\" --ntasks \"${MPI_RANKS}\" ${SOLVER_SRUN_EXTRA_ARGS} \
        --cpus-per-task 1 \
        ${SRUN_DIST} \
        ${SOLVER_EXE} \
        --device "${device}" \
    ...
    SRUN_STATUS=$?
      ...
      srun --wait=120 --export=ALL --gres=none $(get_srun_het_flag "${SOLVER_HET_GROUP}") --nodes "${_nodes:-1}" --ntasks "${MPI_RANKS}" ${SOLVER_SRUN_EXTRA_ARGS} \
        --cpus-per-task 1 \
        ${SRUN_DIST} \
        ${SOLVER_EXE} \
        --device "${device}" \
