...
# CPU ML Scaling Benchmark: Sweep DB_NODES from 1 to 6
...
MANIFEST_FILE="${SCRIPT_DIR}/cpu_scaling_jobs.txt"
...
for db_nodes in $(seq 1 6); do
  ...
  previous_job_id=$(sbatch --parsable \
    ...
    --time="${TIME_LIMIT}" \
    "${RUNNER}")
  print "Submitted ${job_name} (${db_nodes} DB nodes): Job ID ${previous_job_id}"
  print "${db_nodes} direct balanced_control ${previous_job_id}" >> "${MANIFEST_FILE}"
done

print "All 6 CPU scaling jobs submitted successfully in dependency chain."
