#!/bin/zsh
# Run the c=0 default parallel-put baselines for 50k and 600k in one exclusive heterogeneous allocation.

#SBATCH --job-name=CMI_SmartSim_10step_c0_baseline
#SBATCH --account=thes2181
#SBATCH --time=00:30:00
#SBATCH --exclusive
#SBATCH --output=logs/scorep_c0_baseline_%j.txt
...
for batch_size in 50000 600000; do
  ...
  for chains in 0; do
    ...
    run_log="${SCRIPT_DIR}/logs/scorep_c0_baseline_${SUITE_ID}_${case_index}_${batch_label}_c${chains}.log"

    print "Starting ${job_name} (${case_index}/2); log=${run_log}"
    env \
      RUN_ID_ENV="${run_id}" \
      USE_SCOREP_ENV=1 \
