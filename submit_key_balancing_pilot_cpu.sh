...
# 4-job pilot CPU benchmark (216x144 resolution, fast CPU execution):
...
MANIFEST_FILE="${SCRIPT_DIR}/cpu_pilot_jobs.txt"
DB_NODES=6
MPI_RANKS=24
...
print "Submitting CPU 4-job pilot benchmark chain..."
...
for interface in direct cpp; do
  ...
  for variant in balanced_control natural; do
    ...
    job_name="cpupilot_${interface}_${variant}"
    ...
    previous_job_id=$(sbatch --parsable \
      ...
      --time="${TIME_LIMIT}" \
      "${RUNNER}")
    print "Submitted pilot ${interface} ${variant}: Job ID ${previous_job_id}"
    print "1 ${interface} ${variant} ${previous_job_id}" >> "${MANIFEST_FILE}"
  done
done

print "All 4 pilot jobs submitted successfully in dependency chain."
